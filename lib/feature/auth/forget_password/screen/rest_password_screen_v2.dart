...
class ResetPasswordScreenV2 extends StatelessWidget {
  static const routeName = '/rest-password-screen-v2';
  final String? uuid;
  final String? token;
  ResetPasswordScreenV2({
    Key? key,
    this.uuid,
    this.token,
  }) : super(key: key);
  ...
  static open(BuildContext context, String uuid, String token) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ResetPasswordScreenV2(
          uuid: uuid,
          token: token,
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return BlocProvider<ForgetPasswordBloc>(
      ...
      child: RestPasswordScreenWithBlocV2(
        uuid ?? "",
        token ?? "",
      ),
    );
...
class RestPasswordScreenWithBlocV2 extends BaseAuthScreen {
  final String uuid;
  final String token;
  const RestPasswordScreenWithBlocV2(this.uuid, this.token, {super.key});

  @override
  BaseAuthState<RestPasswordScreenWithBlocV2> baseScreenCreateState() {
    return _RestPasswordScreenWithBlocV2();
  }
}

class _RestPasswordScreenWithBlocV2
    extends BaseAuthState<RestPasswordScreenWithBlocV2> with AuthValidate {
  ...
  void _resetPasswordApiEvent() {
    currentBloc
        .add(ResetPasswordApiEventV2(widget.uuid, widget.token, _newPassword));
  }
