...
class SignContractScreenV2 extends StatelessWidget {
  SignContractScreenV2({super.key});

  static const routeName = '/sign-contract-screen-v2';
  ...
  Widget build(BuildContext context) {
    return BlocProvider<SignContractBloc>(
      ...
      child: SignContractScreenWithBloc(
          bookingDetailsModel(context), replacement(context), onBack(context)),
    );
  }

  BookingDetailsModel bookingDetailsModel(BuildContext context) =>
      (ModalRoute.of(context)!.settings.arguments
              as Map)[SignContractScreenV2.argumentApartmentRequestsApiModel]
          as BookingDetailsModel;

  bool replacement(BuildContext context) =>
      (ModalRoute.of(context)!.settings.arguments
          as Map)[SignContractScreenV2.argumentReplacement] as bool;
...
class SignContractScreenWithBloc extends BaseStatefulScreenWidget {
  ...
  const SignContractScreenWithBloc(
      this.bookingDetailsModel, this.replacement, this.onBack,
      {super.key});
      ...
      body: BlocConsumer<SignContractBloc, SignContractState>(
        listener: (context, state) {
          if (state is SignContractLoadingState) {
            showMessageLoading(message: translate(LocalizationKeys.preparingYourContractPleaseWait));
          } else {
            hideMessageLoading();
          }
            ...
            Container(
              ...
              child: AppTextButton(
                  onPressed: () {
                    ...
                    currentBloc.add(SignContractApiEventV2(
                        widget.bookingDetailsModel.bookingId ?? "",
                        _contractResponseModel?.contractID ?? "",
                        file.path));
                    _dismissSignatureBottomSheet();
    ...
    if (widget.replacement) {
      ArrivingDetails.open(
        ...
        ArrivingDetailsRequestModel(
          bookingId: widget.bookingDetailsModel.bookingId ?? "",
          guestId: widget.bookingDetailsModel
                  .guests?[widget.bookingDetailsModel.guestIndex].guestId ??
              "",
        ),
        widget.bookingDetailsModel,
        true,
