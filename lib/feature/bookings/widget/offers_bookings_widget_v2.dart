...
class OffersBookingsWidgetV2 extends StatelessWidget {
  OffersBookingsWidgetV2({Key? key}) : super(key: key);
...
class _BookingsScreenWithBloc extends BaseScreenState<BookingsScreenWithBloc>
  ...
  void initState() {
    super.initState();
    _resetPaging();
    Future.microtask(() => _getoffersBookingsEvent(isSwipeToRefresh: false));
  }
    ...
    return Scaffold(
      body: BlocListener<BookingsBloc, BookingsState>(
        listener: (context, state) {
          if (state is BookingsOffersLoadingState) {
            showLoading();
          } else {
            hideLoading();
          }
  ...
  Widget _wishlistWidget() {
    return BlocBuilder<BookingsBloc, BookingsState>(
      builder: (context, state) {
        if (state is BookingsOffersLoadedStateV2) {
          return _buildListLoaded();
        } else if (state is BookingsOffersLoadingState) {
    ...
    return PagingSwipeToRefreshGridListWidget(
      reachedEndOfScroll: () {
        if (shouldLoadMore) {
          ...
          _getoffersBookingsEvent(isSwipeToRefresh: false);
        }
      },
      ...
      swipedToRefresh: () {
        _resetPaging();
        _getoffersBookingsEvent(isSwipeToRefresh: true);
      },
  ...
  void _getoffersBookingsEvent({required bool isSwipeToRefresh}) {
    currentBloc.add(
        GetOffersBookingsEvent(_pagingInfo.currentPage + 1, isSwipeToRefresh));
  }
