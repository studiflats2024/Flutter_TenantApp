...
import 'package:vivas/feature/bookings/widget/booking_unit_widget_v2.dart';
import 'package:vivas/feature/bookings/widget/no_active_booking.dart';
...
class ActiveBookingsWidgetV2 extends StatelessWidget {
  ActiveBookingsWidgetV2({Key? key}) : super(key: key);
...
class _BookingsScreenWithBloc extends BaseScreenState<BookingsScreenWithBloc>
    with PaginationManager<BookingModel> {
  ...
  void initState() {
    super.initState();
    _resetPaging();
    Future.microtask(() => _getActiveBookingsEvent(isSwipeToRefresh: false));
  }
    ...
    return Scaffold(
      body: BlocListener<BookingsBloc, BookingsState>(
        listener: (context, state) {
          if (state is BookingsActiveLoadingState) {
            showLoading();
          } else {
            hideLoading();
          }
  ...
  Widget _wishlistWidget() {
    return BlocBuilder<BookingsBloc, BookingsState>(
      builder: (context, state) {
        if (state is BookingsActiveLoadedStateV2) {
          return _buildListLoaded();
        } else if (state is BookingsActiveLoadingState) {
    ...
    return PagingSwipeToRefreshGridListWidget(
      reachedEndOfScroll: () {
        if (shouldLoadMore) {
          ...
          _getActiveBookingsEvent(isSwipeToRefresh: false);
        }
      },
      itemWidget: (int index) {
        return BookingUintWidgetV2(
      ...
      swipedToRefresh: () {
        _resetPaging();
        _getActiveBookingsEvent(isSwipeToRefresh: true);
      },
  ...
  void _getActiveBookingsEvent({required bool isSwipeToRefresh}) {
    currentBloc.add(
        GetActiveBookingsEvent(_pagingInfo.currentPage + 1, isSwipeToRefresh));
  }
  ...
  void _unitClickCallback(BookingModel model) {
    RequestDetailsScreenV2.open(context, model.bookingId);
  }
}
